// Complete list of all 114 Surahs with their names and ayah counts
export const surahs = [
...
// Helper function to format surah and ayah range
export const formatSurahAyah = (surahNumber, startAyah, endAyah, endSurahNumber = null) => {
  const surah = getSurahByNumber(surahNumber);
  if (!surah) return '';
  
  if (endSurahNumber && endSurahNumber !== surahNumber) {
    const endSurah = getSurahByNumber(endSurahNumber);
    return `${surah.name} Ayah ${startAyah} - ${endSurah.name} Ayah ${endAyah}`;
  }
